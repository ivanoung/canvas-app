...
function checkChangeFilter() {
  ...
  const canvasFilterStringTemplate = `blur(${
    canvasFilterSetting.blur
  }px)brightness(${canvasFilterSetting.brightness}%) contrast(${
    canvasFilterSetting.contrast
  }%) grayscale(${canvasFilterSetting.grayscale}%) hue-rotate(${
    canvasFilterSetting.huerotate
  }deg) invert(${canvasFilterSetting.invert}%)  opacity(${
    canvasFilterSetting.opacity
  }%) saturate(${canvasFilterSetting.saturate}%) sepia(${
    canvasFilterSetting.sepia
  }%) drop-shadow(${canvasFilterSetting.dropshadow[0]}px ${
    canvasFilterSetting.dropshadow[1]
  }px ${canvasFilterSetting.dropshadow[2]}px ${
    canvasFilterSetting.dropshadow[3]
  })`;
